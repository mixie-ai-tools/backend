...
export class LlmController {
  ...
  async updateChatById(
    ...
    try {
      const assistant = await this.lmStudioService.similaritySearch(
        updateChatDto,
        5,
      );
      ...
      return await this.chatService.updateChatById(
        conversationId,
        updateChatDto,
      );
    } catch (error) {
  ...
  async processDocs() {
    try {
      ...
      return await this.lmStudioService.addShopifyProductsToVectorStore(
        productsData,
      );
    } catch (error) {
      Logger.log(error);
